...
def get_datasetFeaturesFiles(dsname, method): 
    return "./checkpoints/%s/%s/last/novel_features.plk"%(dsname,method)

# --- 加载method骨干网络提取的dsname数据集的验证集，_min_examples为验证集中数量最少的类的数量，保持类别均衡
def loadDataSet(dsname, method):
    ...
    # home = expanduser("~")
    dataset = _load_pickle(get_datasetFeaturesFiles(dsname,method))
...
def setRandomStates(cfg):
    ...
    rsFile = os.path.join(_cacheDir, "RandStates_{}_s{}_q{}_w{}_m{}".format(
        dsName, cfg['shot'], cfg['queries'], cfg['ways'],cfg['method']))
